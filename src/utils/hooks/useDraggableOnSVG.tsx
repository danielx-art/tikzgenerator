...
const useDraggableOnSVG = (
  ...
  const handleMouseDown = useCallback(
    (event: MouseEvent) => {
      ...
      setIsDragging(true);
      const newDrag = {start: initialPosition, diff: {x:0, y: 0}, curr: initialPosition}
  ...
  const handleMouseMove = useCallback(
    (event: MouseEvent) => {
      ...
      const diff = {x: newPosition.x - currentDrag.start.x, y: newPosition.y - currentDrag.start.y}
      const newDrag = {start: currentDrag.start, diff: diff, curr: newPosition } 
      setCurrentDrag(newDrag);
