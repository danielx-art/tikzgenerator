...
function areThereIntersections(edges: Tsegment[]): boolean {
    for (let i = 0; i < edges.length; i++) {
        for (let j = i + 1; j < edges.length; j++) {
            if (segmentsIntersect(edges[i]!.p1, edges[i]!.p2, edges[j]!.p1, edges[j]!.p2)) {
                return true;
            }
        }
    }
...
export function isConnectedGraph(adjacencyList: TAdjacencyList, edges: Tsegment[]): boolean {
    ...
    return Object.keys(adjacencyList).every(vertex => visited[vertex as TpointId]);
}
...
export function findEulerCycle(adjacencyList: TAdjacencyList, edges: TEdges): TEulerPath | null {
    ...
    while (stack.length > 0) {
        ...
        if (adjacencyList[currentVertexId] && adjacencyList[currentVertexId]!.length > 0) {
            // Find an unused edge
            let unusedEdgeId = adjacencyList[currentVertexId]!.find(edgeId => !usedEdges.has(edgeId));
            if (unusedEdgeId !== undefined) {
                ...
                let nextVertexId = edge.p1.id === currentVertexId ? edge.p2.id : edge.p1.id;
...
export const closeFigure = (
    ...
    if (!allVerticesDegreeTwo(adjacencyList)) {
        store.setError("Por favor selecione apenas segmentos conectados um a um (sem que exustam bifurcações). ")
        return undefined;
    }
    if (areThereIntersections(selectedSegments)){ 
        store.setError("Por favor selecione somente segmentos que não se interceptam. ")
        return undefined 
    };
    if (!isConnectedGraph(adjacencyList, selectedSegments)) {
        store.setError("Por favor selecione segmentos que em conjunto contornam uma única região do plano. ")
        return undefined;
    }
    ...
    if(!eulerCycle) {
        store.setError("Não foi possível formar uma figura utilizando os segmentos selecionados. ")
    } else {
