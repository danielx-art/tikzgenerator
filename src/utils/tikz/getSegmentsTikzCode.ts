    ...
    if (segment.visible) {
      ...
      if (segment.marks > 0) {
        ...
        for (let i = 0; i < segment.marks; i++) {
          ...
          tikzCode += `\\draw [${segment.stroke.color}, "solid", line width=${segment.stroke.width*TIKZ_SCALE}pt, opacity=${segment.stroke.opacity}] (${point1.x}, ${point1.y}) -- (${point2.x}, ${point2.y});\n`;
        }
      }
    }
  });
