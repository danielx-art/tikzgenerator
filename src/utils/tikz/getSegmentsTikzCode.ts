...
export default function getSegmentsTikzCode(store: State & Action) {
  ...
  store.segments.forEach((segment) => {
    if (segment.visible) {
      ...
      tikzCode += `\\draw [${segment.stroke.color}, ${strokeStyle}, line width=${segment.stroke.width*store.configs.TIKZ_SCALE}pt, opacity=${segment.stroke.opacity}] (${segment.p1.id}) -- (${segment.p2.id});\n`;
      //DO THE MARKS
      if (segment.marks > 0) {
        const markLength = 0.12 * segment.stroke.width*store.configs.TIKZ_SCALE;
        const markSep = 1.2 * segment.stroke.width*store.configs.TIKZ_SCALE;
        const midPoint = vec()
        ...
        for (let i = 0; i < segment.marks; i++) {
          ...
          tikzCode += `\\draw [${segment.stroke.color}, "solid", line width=${segment.stroke.width*store.configs.TIKZ_SCALE}pt, opacity=${segment.stroke.opacity}] (${point1.x}, ${point1.y}) -- (${point2.x}, ${point2.y});\n`;
        }
      }
    }
  });
