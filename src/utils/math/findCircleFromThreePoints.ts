...
export function findCircleFromThreePoints(p1: vector, p2: vector, p3: vector): { center: vector; radius: number } | null {
    ...
    const slope1 = (p2.x - p1.x) !== 0 ? -(p2.y - p1.y) / (p2.x - p1.x) : Infinity;
    const slope2 = (p3.x - p2.x) !== 0 ? -(p3.y - p2.y) / (p3.x - p2.x) : Infinity;
